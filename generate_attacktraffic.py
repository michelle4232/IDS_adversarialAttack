...
def CreateBatch_GAN(x, batch_size):
    ...
    batch_x = [
        x[batch_size * i : (i + 1) * batch_size, :] for i in range(len(x) // batch_size)
    ]
    return batch_x
...
g_param = th.load("testGAN/generator.pth", map_location=lambda x, y: x)
    ...
    for n, g in model_g.items():
        ...
        with th.no_grad():
            for bn in test_batch_normal:
                ...
                batch_a = th.Tensor(
                    test_raw_attack[
                        np.random.randint(0, len(test_raw_attack), BATCH_SIZE)
                    ]
                )
                z = batch_a + th.Tensor(
                    np.random.uniform(0, 1, (BATCH_SIZE, D_G_INPUT_DIM))
                )

                adversarial_attack = g(z)
                adversarial_attack[:, 33:] = th.Tensor(
                    np.where(
                        adversarial_attack[:, 33:].detach().cpu().numpy() >= 0.5, 1, 0
                    )
                )
                ...
                pred_label_adv = (
                    th.argmax(nn.Sigmoid()(ids_pred_adv), dim=1).cpu().numpy()
                )
                pred_label_ori = (
                    th.argmax(nn.Sigmoid()(ids_pred_ori), dim=1).cpu().numpy()
                )

                tn1, fp1, fn1, tp1 = confusion_matrix(
                    ids_true_label, pred_label_adv
                ).ravel()
                tn2, fp2, fn2, tp2 = confusion_matrix(
                    ids_true_label, pred_label_ori
                ).ravel()
        ...
        avg_eir = np.mean(eir)
        print(
            f"{n} => origin_DR : {avg_odr} \t adversarial_DR : {avg_adr} \t EIR : {avg_eir}"
        )
        if n == "learned":
